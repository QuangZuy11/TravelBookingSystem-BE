...
exports.getProviderDashboardStats = async (req, res) => {
  try {
    ...
    const stats = await Promise.all([
      // Tour Stats - ✅ Chỉ tính tour có status là 'published'
      Tour.aggregate([
        {
          $match: {
            provider_id: new mongoose.Types.ObjectId(providerId),
            status: "published", // ✅ Chỉ tính tour published
          },
        },
        {
          $group: {
      ...
      // Popular Tours (based on rating) - ✅ Chỉ hiển thị tour published
      Tour.aggregate([
        {
          $match: {
            provider_id: new mongoose.Types.ObjectId(providerId),
            status: "published", // ✅ Chỉ hiển thị tour published
          },
        },
...
exports.createTour = async (req, res) => {
  ...
  try {
      ...
      fieldsToParseAsJSON.forEach((field) => {
        if (tourData[field] && typeof tourData[field] === "string") {
          try {
            ...
            if (
              tourData[field].trim().startsWith("{") ||
              tourData[field].trim().startsWith("[")
            ) {
    ...
    if (req.file) {
      ...
      throw new Error(
        "Tour image is required - provide either a file upload or image URL"
      );
    }
...
exports.getAllProviderTours = async (req, res) => {
  try {
    ...
    // ✅ Mặc định chỉ hiển thị tour có status là 'published'
    // Nếu provider muốn xem tour draft hoặc archived, họ có thể filter qua query parameter
    if (status) {
      query.status = status;
    } else {
      // Mặc định chỉ hiển thị tour published nếu không có filter status
